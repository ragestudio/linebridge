...
struct WebSocketWrapper {
	...
	template <bool SSL>
	static uint32_t uWS_WebSocket_send_fast_string(v8::Local<v8::Object> receiver, const v8::FastOneByteString &message, bool isBinary, bool compress) {
		auto *ws = (uWS::WebSocket<SSL, true, PerSocketData> *)getInternalPointer(receiver);  //->GetAlignedPointerFromInternalField(0);
		if (!ws) return 0;
		return ws->send(std::string_view(message.data, message.length), isBinary ? uWS::OpCode::BINARY : uWS::OpCode::TEXT, compress);
	}

	// Version B: Handles ArrayBuffer/TypedArray
	template <bool SSL>
	static uint32_t uWS_WebSocket_send_fast_buffer(v8::Local<v8::Object> receiver, v8::Local<v8::Value> message, bool isBinary, bool compress) {
		auto *ws = (uWS::WebSocket<SSL, true, PerSocketData> *)getInternalPointer(receiver);  //->GetAlignedPointerFromInternalField(0);
		if (!ws) return 0;
		...
		return ws->send(std::string_view(data, length), isBinary ? uWS::OpCode::BINARY : uWS::OpCode::TEXT, compress);
	}
