...
/* PROTOCOL is 0 = TCP, 1 = TLS, 3 = CACHE */
...
struct HttpResponseWrapper {
	...
	static void res_upgrade(const FunctionCallbackInfo<Value> &args) {
		...
		if (res) {
			...
			assumeCorked();
			res->template upgrade<PerSocketData>({ std::move(userData) }, secWebSocketKey.getString(), secWebSocketProtocol.getString(), secWebSocketExtensions.getString(), context);
