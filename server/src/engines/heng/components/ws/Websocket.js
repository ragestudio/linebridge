...
import { array_buffer_to_string } from "../../shared/operators.js"
...
export default class Websocket extends EventEmitter {
	...
	publish(topic, message, is_binary, compress) {
		return this.#ws
			? this.#ws.publish(topic, message, is_binary, compress)
			: false
	}
		...
		if (this.#ws) {
			...
			switch (type) {
				case FRAGMENTS.FIRST:
					sent = this.#ws.sendFirstFragment(
						chunk,
						is_binary,
						compress,
					)
			...
			if (sent) {
				...
				this.once("drain", () =>
					this._write(type, chunk, is_binary, compress, callback),
				)
			}
	...
	_stream_chunk(stream, type, chunk, is_binary) {
		...
		if (!sent) {
			...
			this.once("drain", () =>
				this._stream_chunk(stream, type, chunk, is_binary),
			)
	...
	stream(readable, is_binary = true) {
		...
		if (!(readable instanceof Readable))
			throw new Error(
				"Websocket.stream(readable) -> readable must be a Readable stream.",
			)
