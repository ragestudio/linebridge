...
export default class WebsocketRoute extends Route {
	...
	_get_message_parser(type) {
		switch (type) {
			...
			case "Buffer":
				...
				return (array_buffer) =>
					Buffer.concat([Buffer.from(array_buffer)])
			case "ArrayBuffer":
	...
	_create_uws_route() {
		...
		uws_options.close = (ws, code, message) =>
			this._on_close(ws, code, message)
		uws_options.message = (ws, message, isBinary) =>
			this._on_message(ws, message, isBinary)
