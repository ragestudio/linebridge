...
export default class ExpressRequest {
	...
	accepts(types) {
		...
		const mimes = arrayTypes.map((type) =>
			type.indexOf("/") === -1 ? mime_types.lookup(type) : type,
		)
		const first = this.#negotiator.mediaType(
			mimes.filter((type) => typeof type === "string"),
		)
	...
	param(name, default_value) {
		...
		if (
			...
			path_parameters.hasOwnProperty(name)
		)
