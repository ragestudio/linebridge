...
export default class Route {
	...
	constructor({ app, method, pattern, options, handler }) {
		...
		this.max_body_length =
			options.max_body_length || app._options.max_body_length
		...
		const wildcard =
			pattern.includes("*") || this.path_parameters_key.length > 0
	...
	handle(request, response, cursor = 0) {
		...
		const is_async_handler =
			(middleware ? middleware.handler : this.handler).constructor
				.name === "AsyncFunction"
		if (is_async_handler) {
	...
	compile() {
		...
		Object.keys(app_middlewares).forEach((pattern) =>
			app_middlewares[pattern].forEach((middleware) => {
				...
				const indirect_child =
					middleware.pattern.startsWith(wildcard_path)
		...
		const offset = middlewares.reduce(
			(max, middleware) => (middleware.id > max ? middleware.id : max),
			0,
		)
