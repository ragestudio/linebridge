...
export default class Router {
			...
			if (i === 1) {
				if (typeof argument === "string") {
					if (this.#context_pattern) {
						...
						pattern = merge_relative_paths(
							this.#context_pattern,
							argument,
						)
					} else {
						// The path is as is
	...
	use() {
		...
		for (let i = 0; i < arguments.length; i++) {
			...
			if (typeof candidate == "function") {
				...
				// Scenario: Array of functions
				candidate.forEach((middleware) =>
					this._register_middleware(pattern, middleware),
				)
			} else if (
				typeof candidate == "object" &&
				candidate.constructor.name === "Router"
			) {
	...
	route(pattern) {
		...
		if (!pattern || typeof pattern !== "string")
			throw new Error(
				"HyperExpress.Router.route(pattern) -> pattern must be a string.",
			)
