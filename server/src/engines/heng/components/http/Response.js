...
export default class Response {
	...
	_track_middleware_cursor(position) {
		...
		if (
			this._middleware_cursor === undefined ||
	...
	_initiate_response() {
		...
		if (this._status_code || this._status_message)
			this._raw_response.writeStatus(
				this._status_code +
					" " +
					(this._status_message || STATUS_CODES[this._status_code]),
			)
		...
		if (this._cookies) {
			for (const name in this._cookies) {
				this._raw_response.writeHeader(
					"set-cookie",
					this._cookies[name],
				)
			}
	...
	_writev(chunks, callback, index = 0) {
		...
		this._write(chunks[index], null, (error) => {
			...
			if (typeof chunks[index].callback == "function")
	...
	send(body, close_connection) {
		...
		if (!this.completed) {
			...
			if (this._writable && !this._streaming)
	...
	async stream(readable, total_size) {
		...
		if (!this.completed) {
			...
			this.once("close", () =>
				!readable.destroyed ? readable.destroy() : null,
			)
			...
			while (
				!this.completed &&
				!(readable.readableEnded || readable.destroyed)
			) {
	 ...
	 */
	json(body) {
		return this.header("content-type", "application/json", true).send(
			JSON.stringify(body),
		)
	}
	...
	throw(error) {
		...
		if (!(error instanceof Error))
			error = new Error(`ERR_CAUGHT_NON_ERROR_TYPE: ${error}`)
	...
	method: (type, name, original) => {
		...
		const passthrough = function () {
			...
			if (this._writable === null) {
				...
				this._writable = new stream.Writable(
					this.route.streaming.writable,
				)
