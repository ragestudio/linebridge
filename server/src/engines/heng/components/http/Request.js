...
import MultipartField from "../plugins/MultipartField.js"
import NodeRequest from "../compatibility/NodeRequest.js"
import ExpressRequest from "../compatibility/ExpressRequest.js"
...
export default class Request {
	...
	constructor(route, raw_request) {
		...
		this._method =
			route.method !== "ANY" ? route.method : raw_request.getMethod()
			...
			if (!limited) {
				...
				switch (this._body_parser_mode) {
					...
					case 0:
						...
						if (
							this._body_received_bytes >
							this.app._options.max_body_buffer
						)
							this.pause()
						break
	...
	multipart(options, handler) {
		...
		if (!/^(multipart\/.+);(.*)$/i.test(content_type))
		...
		return new Promise((resolve, reject) => {
			...
			const finish = async (error) => {
				...
				if (error && !silent_error) {
					...
					if (reference._multipart_promise)
						await reference._multipart_promise
			...
			const on_field = (name, value, info) => {
				...
				reference
					._on_multipart_field(handler, name, value, info)
					.catch(finish)
			}
