...
import HostManager from "./plugins/HostManager.js"
import WebsocketRoute from "./ws/WebsocketRoute.js"

import { wrap_object, to_forward_slashes } from "../shared/operators.js"
...
export default class Server extends Router {
	...
	constructor(options = {}) {
		...
		try {
			...
			if (this.#options.is_ssl) {
				...
				this.#options.cert_file_name = to_forward_slashes(
					path.resolve(cert_file_name),
				)
				this.#options.key_file_name = to_forward_slashes(
					path.resolve(key_file_name),
				)
	...
	async listen(first, second, third) {
		...
		if (this.#options.is_ssl) {
			...
			try {
				...
				await Promise.all([
					fs.access(key_file_name),
					fs.access(cert_file_name),
				])
			} catch (error) {
				throw new Error(
		...
		return await new Promise((resolve, reject) => {
			...
			const on_listen_socket = (listen_socket) => {
				...
				if (listen_socket) {
					...
					if (reference.#options.auto_close)
	...
	shutdown(listen_socket) {
		...
		if (!this.#pending_requests_count)
			return Promise.resolve(this.close(listen_socket))
	...
	#handlers = {
		...
		on_error: (request, response, error) => {
			...
			return response
				.status(500)
				.send("HyperExpress: Uncaught Exception Occured")
		},
	}
	...
	_create_route(record) {
		...
		switch (method) {
			...
			default:
				...
				return this.#uws_instance[method](
					pattern,
					(response, request) => {
	...
	_create_middleware(record) {
		...
		if (this.#middlewares[pattern] == undefined)
			this.#middlewares[pattern] = []
	...
	_compile() {
		...
		Object.keys(this.#routes).forEach((method) =>
			Object.keys(this.#routes[method]).forEach((pattern) =>
				this.#routes[method][pattern].compile(),
			),
		)
