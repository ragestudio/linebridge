...
function inherit_prototype({
	...
	if (Array.isArray(from))
		return from.forEach((f) =>
			inherit_prototype({ from: f, to, override, method, ignore }),
		)
	...
	Object.keys(from_descriptors).forEach((name) => {
		...
		if (typeof override == "function" && to_descriptors[name]?.value)
		...
		if (typeof value === "function") {
			...
			if (typeof get === "function")
				definition.get = method("GETTER", name, get)

			// Initialize a middleman setter method
			if (typeof set === "function")
				definition.set = method("SETTER", name, set)

			// Inject the definition into the "to" prototype
			if (definition.get || definition.set)
				Object.defineProperty(to, name, definition)
		}
	})
}
